...
#include "Assets/Shaders/Library/Math.hlsl"
...
float WorldDither4( float3 positionWS )
{
    int2 uv = int2( mod( positionWS.xz * 16, 2.0) );
    return bayer4[int(uv.x) + int(uv.y) * 2];
}

float WorldDither16( float3 positionWS )
{
    int2 uv = int2( mod( positionWS.xz * 16, 4.0) );
    return bayer16[uv.x + uv.y * 4];
}

float WorldDither64( float3 positionWS )
{
    int2 uv = int2( mod( positionWS.xz * 16, 8.0 ) );
